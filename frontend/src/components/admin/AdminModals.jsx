...
export const BulkUploadModal = ({ isOpen, onClose, type, onUploadSuccess }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      console.error(err);
      let errMsg = err.response?.data?.detail || err.message || "An unexpected error occurred.";
      if (typeof errMsg === "string" && (errMsg.toLowerCase().includes("timeout") || errMsg.toLowerCase().includes("exceeded"))) {
        errMsg = "Upload is taking longer than expected. Please try with smaller file or wait and check records.";
      }
              ...
              >
                {uploading ? (
                  <>
                    ...
                    Uploading and processing records... This may take up to 1 minute.
                  </>
                ) : (
                  "Upload & Ingest"
                )}
